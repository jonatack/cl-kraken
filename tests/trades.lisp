...
(deftest trades
  (testing "when passed \"xbtUSD\", evaluates to data on 1000 XBTUSD trades"
    (let* ((response         (cl-kraken:trades "xbtUSD"))
           ...
           (volume           (second trade))
           (time             (third trade))
           (buy-or-sell      (fourth trade))
           (market-or-limit  (fifth trade))
           (miscellaneous    (sixth trade)))
      ...
      (ok (typep time 'ratio))
      (ok (or (string= buy-or-sell "b") (string= buy-or-sell "s")))
      (ok (or (string= market-or-limit "m") (string= market-or-limit "l")))
      (ok (string= miscellaneous ""))))
